...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func handle(key : String) {
        DispatchQueue.main.async {
            ...
            if key == "das" {
                controller = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "PageViewController")
            } else {
                controller = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "ViewController")
            }
            if let nav = self.window?.rootViewController as? UINavigationController , let vc = controller {
